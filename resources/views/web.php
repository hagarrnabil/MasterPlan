...
Route::get('/' ,function (Request $request) {
    ...
    $buildings = $buildings->where('buildingCode' , '=' , $request->input('buildingCode') );
		}
		if (!empty($request->input('zone'))){
    $buildings = $buildings->where('zone' , '=' , $request->input('zone') );
		}
		if (!empty($request->input('type'))){
    $buildings = $buildings->where('type' , '=' , $request->input('type') );
		}
		if (!empty($request->input('phase'))){
    $buildings = $buildings->join('project' , '	projectCode')->where('phase' , '=' , $request->input('phase') );
		}
    ...
    $buildings = $buildings->where('size' , '>=' , $request->input('sizeFrom') );
		}
		if (!empty($request->input('sizeTo'))){
    $buildings = $buildings->where('size' , '<=' , $request->input('sizeTo') );
		}
		if (!empty($request->input('priceFrom'))){
    $buildings = $buildings->where('priceAmount' , '>=' , $request->input('priceFrom') );
		}
		if (!empty($request->input('priceTo'))){
    $buildings = $buildings->where('priceAmount' , '<=' , $request->input('priceTo') );
		}
		
	}
	$buildings = $buildings->distinct('buldingCode')->get();
	dd($buildings);
